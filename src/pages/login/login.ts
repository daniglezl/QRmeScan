  ...
  */
  doLogin() {
    if(this.showLogin) {
      ...
      this.auth.login('custom', {'email':this.email, 'password':this.password}, { 'inAppBrowserOptions': {'hidden': true} }).then(() => {
        ...
        let errors = '';
        if(err.message === 'unauthorized') errors += 'Email or password are not valid.<br/>';
